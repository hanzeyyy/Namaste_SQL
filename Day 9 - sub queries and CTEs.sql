...
/* 
this value is wrong since each order can have multiple products which needs to be sumed 
and then taken in consideration for average
e.g. let's say
...
--we can say inner query inside close parentheis and 
--outer query (main/outer) outside close parenthesis

select * from employee
where dept_id not in (100,200,300);
--we can get only one result from inner query
...
select *, 
	(select avg(salary) from employee) as avg_sales 
from employee;
...
select emp.*, dep.avg_dept_salary 
from employee emp
inner join
...
select * from employee;
--instead of using dep.avg_dept_salary, 
--if we use dep.* then gives us an additinal column in the output table as dept_id (which is unnecessory)
select emp.*, dep.* 
from employee emp
inner join
	(select dept_id, avg(salary) as avg_dept_salary
...
select emp.*, dep.* from 
employee emp
inner join dep on emp.dept_id = dep.dept_id;
...
select *, 
	(select avg(salary) from employee) as avg_sales from employee
where dept_id in (select dep_id from dept);
